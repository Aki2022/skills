...
**ゲートが承認されるまで、資産値（tokens実値・フォント・ロゴ・テンプレート・境界データ）を
成果物に反映してはいけない。** 本箱は 2026-07-04 に承認済みで、`plugin.yaml` の
`usage_policy.license_gate_approved` は `true`（承認記録は本ファイル末尾）。

<!-- 2026-09-09: 上の記述は承認前の文面（「`false` のまま」）が残っていたため現状に合わせた。
     frontmatter の license_gate_status: APPROVED と plugin.yaml の true が正で、散文が古かった。 -->

#!/usr/bin/env python3
"""check_brand_metadata.py — own-brand-resolve/plugins/*/plugin.yaml の等級（tier）と
取得メタ（source）の形を検査する。
...
使い方:
  ...
  （省略時は own-brand-resolve/plugins を既定値として使う）
"""
...
# own-brand-resolve/scripts/check_brand_metadata.py から見た正典パス
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        help="箱（<box>/plugin.yaml）を並べたディレクトリ（既定: own-brand-resolve/plugins）",
    )

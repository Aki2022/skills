...
own-pptx-build/style-guide/tokens.json の現行値が同値かを検査する。

決定6 により、倉庫 my_company の pptx 媒体プロファイルは own-pptx-build の tokens.json
...
# own-brand-resolve/scripts/check_tokens_parity.py から見た正典パス
...
DEFAULT_PPTX_TOKENS = _REPO_ROOT / "own-pptx-build" / "style-guide" / "tokens.json"

# (表示名, 倉庫側ファイル種別, 倉庫側パス, own-pptx-build側パス)
# これだけを比べる。増やさない。
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        help="own-pptx-build/style-guide/tokens.json のパス",
    )

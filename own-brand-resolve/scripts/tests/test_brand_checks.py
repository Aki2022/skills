...
own-brand-resolve/plugins/*/plugin.yaml の「等級（tier）と取得メタ（source）」の形と、
倉庫 my_company の pptx 媒体プロファイル ↔ own-pptx-build/style-guide/tokens.json の
...
REPO_ROOT = SCRIPTS_DIR.parents[1]  # .../own-brand-resolve/scripts -> parents[1] = リポジトリルート

METADATA_SCRIPT = SCRIPTS_DIR / "check_brand_metadata.py"
PARITY_SCRIPT = SCRIPTS_DIR / "check_tokens_parity.py"

CANON_PLUGINS_DIR = REPO_ROOT / "own-brand-resolve" / "plugins"
...
CANON_PPTX_TOKENS = REPO_ROOT / "own-pptx-build" / "style-guide" / "tokens.json"

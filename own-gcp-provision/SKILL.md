...
> その場合は `bash ~/.agents/skills/own-gcp-provision/re-auth.sh` を使い、
...
| `bq: Reauthentication failed`                                                                         | 土台のユーザー認証が期限切れ（RAPT など）                                    | `bash ~/.agents/skills/own-gcp-provision/re-auth.sh` を実行                                   |

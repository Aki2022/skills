...
```bash
bash ~/.agents/skills/own-git-clean/scripts/survey.sh --repo <repo path>
```
...
_against_ a repository — `own-ws-drain`, `own-session-close` — the current
...
```bash
python3 ~/.agents/skills/own-git-clean/scripts/check_active_issue_branches.py <repo path>
```

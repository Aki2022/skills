...
so no later run re-asks them. Route to `own-spec-grill` instead only when the gap
...
1. If a matching goal exists, inspect it and resume it without replacing its
   objective or budget.
2. Treat an explicit `$own-goal-run` request to run the workstream as permission

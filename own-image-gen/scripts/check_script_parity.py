...
（片方だけ直してもう片方を直し忘れる、を防ぐ）:
  ...
  - own-image-gen/scripts/collect_codex_images.py
  - own-pptx-build/scripts/collect_codex_images.py
...
# own-image-gen/scripts/check_script_parity.py から見た正典パス
...
DEFAULT_PPTX_COPY = _REPO_ROOT / "own-pptx-build" / "scripts" / "collect_codex_images.py"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--image-gen", default=str(DEFAULT_IMAGE_GEN_COPY),
                     help="own-image-gen 側 collect_codex_images.py のパス")
    ap.add_argument("--pptx", default=str(DEFAULT_PPTX_COPY),
                     help="own-pptx-build 側 collect_codex_images.py のパス")

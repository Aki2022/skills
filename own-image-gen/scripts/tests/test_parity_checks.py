...
段階移行の期間、own-image-gen と own-pptx-build には「同じもの」が二重に存在する:
...
REPO_ROOT = SCRIPTS_DIR.parents[1]  # .../own-image-gen/scripts -> parents[1] = リポジトリルート

SHELL_SCRIPT = SCRIPTS_DIR / "check_shell_parity.py"
SCRIPT_PARITY_SCRIPT = SCRIPTS_DIR / "check_script_parity.py"

CANON_SHELL = REPO_ROOT / "own-image-gen" / "references" / "prompt-shell.md"
CANON_CONVENTION = REPO_ROOT / "own-pptx-build" / "style-guide" / "imagegen-prompt-convention.md"

CANON_COPY_A = REPO_ROOT / "own-image-gen" / "scripts" / "collect_codex_images.py"
CANON_COPY_B = REPO_ROOT / "own-pptx-build" / "scripts" / "collect_codex_images.py"
...
class CheckScriptParityTest(unittest.TestCase):
    @unittest.skipUnless(
        ...
        "（親セッションが own-image-gen 側を並行して配置中のため）。"
        "両方揃った時点で親が再実行する。",
    )

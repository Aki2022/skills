...
description: >
  ...
  「厳しく評価して」「初見の読者として読んで」で使う。工場 skill（own-web-design / own-pptx-build / own-website-audit）
  の工程4から呼ばれるほか、工場を経由しない単発の作業でも直接呼べる。ルーブリック・ペルソナ・合格ラインは
  呼び出し側が文脈として上乗せする。使わない場面: どの検査 skill を並べるかの振り分け（own-design-route
  が持つ）、指摘を受けた成果物の修正そのもの（呼び出し側が行う）、成果物の生成（各工場 skill が持つ）。
---

# own-output-eval — 合格まで回す評価ループ（共通層）

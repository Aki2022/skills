...
## A. 工場から呼ぶ（例: `own-web-design` 工程4 — 会社サイトのイラスト5枚と LP）
...
| ルーブリック | `rubric.template.md` を埋めたもの。Style 観点の値は `own-brand-resolve` の様式、禁則は web の媒体適合 | 工場（倉庫から取った値を上乗せ） |
...
**工場が直す** → 工場が再度呼ぶ（同じ `eval_dir`）→ … → 最終報告 → 工場が `own-goal-run` へ渡す → 人間ゲート。

...
description: >
  ...
  を置き換える現行スキル（旧方式は廃止済み）。**組織標準のデッキを作る**とき
  （新規作成と、その成果物の改訂）は必ず本 skill を使う。
  使わない場面: 任意の .pptx を**読む・テキストを抽出する・機械的に編集する**、
  テンプレート/スピーカーノート/コメントの操作、分割結合
  （これらは組み込みの pptx skill が持つ。本 skill は組織標準の生成パイプライン）。
---

# own-pptx-build — ハイブリッド PPTX 生成スキル
...
**コンテンツ評価ループ（計画承認制・①テキスト段階〜②.5画像段階）**: base.md／outline.md／
画像の各段階で、**審査員＋初見読者**の subagent 読者評価を基準点まで反復できる
（`references/content-eval.md`・設定は `skill-config.json` `contentEval`）。発火は
**計画承認制**——対象・評価者・モデル・回数・**コスト概算**を人間に提示し、承認後にのみ実行。
**読者評価の入力は「その段階で読み手が受け取る成果物・単独」**（画像段階は画像のみ・
.md 併給禁止）。初見読者の「分からない」は must-fix 相当（審査員だけのループは可読性の
劣化を検出できない）。修正フェーズには**アドバイザー（ゼロベース設計者）**を fail 箇所へ投入。
指摘は正典（outline.md/base.md）と照合し、**正典自体の変更に及ぶものは gate＝人間確認を
経てから正典を更新する**（スライド内で閉じる修正だけをループ内で処理）。
**AI 審査の合格は①②④.5 の人間ゲートの代理にならない。**
④のVLM比較は「承認モックアップ通りに施工されたか」しか見ないため、構図・ロジック級の
差し戻しは②.5 がなければ⑤まで残り、③の実装ごと捨てることになる。
詳細は **`references/content-eval.md`**。
...
**コンテンツ評価（AIによる品質評価・計画承認制）とは別物**で、AI評価を実施しても ④.5 は省略できない。
...
- `references/content-eval.md` — **コンテンツ評価ループ**（計画承認制・base/outline/画像の各段階）: 審査員＋初見読者の2役・修正時アドバイザー・読者評価の対象単独規約（画像段階は画像のみ）・閾値/回数既定・フレッシュ評価・正典照合ルール（gate）・出力スキーマ

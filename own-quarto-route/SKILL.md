---
name: own-quarto-route
description: Quarto 作業の入口。`.qmd`・`_quarto.yml`・Quarto プロジェクト・RevealJS スライド・Quarto からの PowerPoint 出力に関わる依頼で必ず最初に使う。「Quarto で作って」「qmd を直して」「スライドを Quarto で」「Quarto をレンダリングして」「callouts / cross-reference / citation の書き方」「R Markdown（bookdown / blogdown / xaringan / distill / Jupyter）から Quarto へ移行」で発火する。依頼の形を判定して `own-quarto-build`（組織テンプレートからの成果物作成・レンダリング）と `quarto-authoring`（Quarto の機能・構文リファレンスと他フォーマットからの移行）へ振り分け、使わなかった側を理由つきで報告する。Quarto 系 skill が2つあってどちらか迷う場面では、個別 skill を直接選ばずまずこれを使う。使わない場面: Quarto を伴わない Markdown/PDF/PPTX 作業（`own-pptx-build` 等）、Quarto と無関係な R コード作業（`own-r-author`）。
---
...
|          | `own-quarto-build`                                                                                                              | `quarto-authoring`                                                                                                                                   |
| -------- | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| 依頼の形                                                                                                                                                    | 担当                                                                                 |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| 新規の成果物を作る（文書・スライド・PowerPoint 出力）                                                                                                       | `own-quarto-build`（組織テンプレートの複製が先）                                        |
| 既存 Quarto プロジェクトの体裁・出力構成を変える                                                                                                            | `own-quarto-build`（既存設定を先に確認し、既存デザインを勝手に置換しない）              |
...
| レンダリング失敗の切り分け                                                                                                                                  | 構文・YAML はまず `quarto-authoring`、拡張やテンプレートのパス解決は `own-quarto-build` |

迷ったら聞かずに「成果物作成として扱う」と宣言して `own-quarto-build` から始める
（組織テンプレートを外して作ってしまう方が、後から直す費用が高いため）。

### 2. 実行順序

成果物作成では **`own-quarto-build` が先**。テンプレートを destination へ置いてから中身を書く。
途中で機能・構文の疑問が出たら `quarto-authoring` の該当 `references/` を読み、
`own-quarto-build` の Validation（拡張・テーマ・CSL・reference document のパス解決、
...
- **このルーターを経由しない直接発火は防げない。** 2 つの skill の description は
  どちらも Quarto で発火するままで（`own-quarto-build` は `any Quarto document` と広い）、
...
- `quarto-authoring` は外部由来のため、**そちらに境界を書いても上流更新で上書きされる**。
  境界を持てるのは自前側（この skill と `own-quarto-build`）だけ。

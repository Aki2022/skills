...
2. **補助: セッション終了時の棚卸し。** `own-session-close` の手順に組み込み、
   指摘されなかったトラブルを拾う。
...
```bash
python3 ~/.agents/skills/own-trouble-log/scripts/status_ledger.py --root "$ROOT" sync
python3 ~/.agents/skills/own-trouble-log/scripts/status_ledger.py --root "$ROOT" summary
python3 ~/.agents/skills/own-trouble-log/scripts/status_ledger.py --root "$ROOT" validate
```

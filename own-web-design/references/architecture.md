...
| **Design/Asset Plugin** | `own-brand-resolve/plugins` の各箱（別 skill）        | ブランド・デザインシステム固有の DESIGN.md / tokens / assets / templates | 媒体固有の実装制約                 |
...
User Task
  ...
  → 倉庫 own-brand-resolve に問い合わせる（選択規則・extends・ライセンスゲートは倉庫が持つ）
...
工場は起動時に `media/*/media.yaml` をスキャンして媒体一覧を作る。意匠の箱の一覧は倉庫が持つ。

...
description: >-
  ...
  continuous run: pick the next workstream, execute it via own-goal-run, close
  it via own-session-close (merging green PRs autonomously under the
...
1. **Inventory.** Read `docs/00_index.md` and list active workstreams with
   ...
   **Then reconstruct the review shelf from open PRs**, before selecting
   ...
   dated sibling a name collision forces (§1.6 of `own-goal-run`), re-select the
   ...
   **leftovers**, in the inventory and the journal. They are not shelved and do
   ...
   `own-goal-run`'s stale-branch rule at execution time, which salvages, cuts from
   ...
   second commit and break `own-session-close`'s one-self-consistent-commit
...
2. **Merge policy.** Confirm how PRs land. The default is continuous delivery:
   ...
   `own-session-close` — this is what keeps the loop looping. Honor a
...
2. **Execute** via `own-goal-run`, through the workstream's consecutive ready
...
3. **Close** via `own-session-close`. Under the CD merge policy (the
...
- **Question gate** — a decision, missing input, or new permission is needed
  ...
  workstream first (`own-goal-run` already does this), close out the current
...
次のアクション: <single next step, e.g. "review the shelf PRs, then rerun /own-ws-drain">
```

## Token efficiency

- Per iteration read only `docs/00_index.md` and the selected workstream (plus
  what `own-goal-run` itself requires). Never rescan all of `docs/`.
...
- Route read-heavy inventory or search subtasks to cheaper model tiers, as
  `own-goal-run` already prescribes for subagents.
